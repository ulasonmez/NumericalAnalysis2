...
float fonksiyonSonuc(float xDegeri){
	float e = exp(1);
	return pow(e,-0.5*(xDegeri))-xDegeri-0.2;
}
...
float fonksiyonTurevSonuc(float xDegeri){
	float e = exp(1);
	return (-0.5*(xDegeri))*pow(e,(-0.5*(xDegeri))-1)-1;
}
int main(int argc, char *argv[]) {
	...
	if(fonksiyonSonuc(altAralik)!=0 && fonksiyonSonuc(ustAralik)!=0){
		...
		printf("\n\n1. Metot = Ikiye bolme metodu ile 0.02 den daha kucuk hata degerine %d. adimda ulasilmistir.\n",ibKontrol);
		printf("2. Metot = Newton Thompson metodu ile 0.02 den daha kucuk hata degerine %d. adimda ulasilmistir.\n",ntKontrol);
		printf("3. Metot = Dogrusal Interpolasyon metodu ile 0.02 den daha kucuk hata degerine %d. adimda ulasilmistir.\n",diKontrol);
